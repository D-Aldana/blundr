...
- The report payload carries two fields beyond Section 14's shape. `summary_source` (`"llm"` or `"fallback"`) makes guardrail catches and budget exhaustion visible during testing per Section 11; it is not surfaced in the UI, since both paths state the same computed facts, so which one wrote the paragraph is an implementation detail rather than something a reader needs disclosed. Each recommendation also carries `pattern`, the one sentence on where that weakness concentrates (empty when it concentrates nowhere, or when the only lopsided axis is one the category's own trigger already implies — an endgame mistake is late by construction, so "after move 30" is never said there) — its own field rather than a suffix on `evidence`, so the report can place and style it apart from the raw numbers. The summary's facts and the deterministic fallback join the two themselves.
